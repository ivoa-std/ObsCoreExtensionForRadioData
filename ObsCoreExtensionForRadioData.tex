...
ObsCore attributes \emph{em\_min, em\_max} are expressed in wavelength and are not much used within the radioastronomical community.
Therefore a mechanism for their translation into frequency should be provided, then avoiding duplication of the same spectral information . 
...
%median
one computed at the receiver nominal frequency) for these two parameters can be given. It is
...
% see the proposal added in Markus review's pull request 
As maps they are
not queriable. So links to these kind of maps will not be exposed in the extension
table but only via a DataLink service.

Data providers may want to expose some other useful instrumental parameters in order to facilitate data discovery.  For instance important features are the antenna diameter (or maximum antenna diameter), the number of
...
measurements made in a given sequence of time. \emph{obs\_id} should be attributed by the data provider to each observation, which as a mandatory condition should be unambiguously assigned. 
...
It has to be unique in the Virtual Observatory domain and its value should be assigned by the data provider. 
...
A new external standard IVOA vocabulary is currently defined for data product types \footnote{https://www.ivoa.net/rdf/product-type.html}
...
The largest angular scale is also variable along the spectral range. Thus it is best described using \emph{s\_largest\_angular\_scale\_min} and \emph{s\_largest\_angular\_scale\_max} estimated as  respectively $\lambda\_min/l$ and  $\lambda\_max/l$.
...
\emph{instr\_tel\_diameter, instr\_feed } are valid also for SD (see Table \ref{tab:ExtensionAtt_instrumental} for definitions).
