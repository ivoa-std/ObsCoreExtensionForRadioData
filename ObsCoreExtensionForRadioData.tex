...
and FRB Radio Data Discovery and Access IVOA Note\footnote{\url{https://wiki.ivoa.net/internal/IVOA/RadioastronomyInterestGroupFifthVirtualMeeting/PulsarRadioDataAccess.pdf}}\todo{I'm totally not wild about citing an IVOA note that's not on the doc repo.  Upload it now, it's just a note!}.
...
The ObsCore parameter \emph{t\_resolution} (see Sect. \ref{subsec:sd}) has limited applicability to SD data
...
The largest angular scale is also variable along the spectral range. This is why we bound\todo{What does ``bound'' mean here?  Why the past tense?} it with \emph{s\_largest\_angular\_scale\_min} and \emph{s\_largest\_angular\_scale\_max} estimated as  respectively $\lambda\_min/l$ and  $\lambda\_max/l$
